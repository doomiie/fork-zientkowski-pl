...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check($_POST['csrf_token'] ?? '')) {
        ...
        if ($orderId <= 0) {
            $error = 'Niepoprawny identyfikator zamówienia.';
        } else {
            try {
                if ($action === 'mark_paid') {
                    ...
                    $ok = 'Zamówienie oznaczone jako opłacone i zaksięgowane.';
                } elseif ($action === 'mark_failed') {
                    $pdo->prepare("UPDATE token_orders SET status='failed', updated_at=NOW() WHERE id=?")->execute([$orderId]);
                    $ok = 'Zamówienie oznaczone jako failed.';
                } elseif ($action === 'grant') {
                    if (vt_grant_order_entitlements($pdo, $orderId)) $ok = 'Zaksięgowano uprawnienia.';
                    else $error = 'Nie udało się zaksięgować uprawnień (sprawdź status).';
                }
            } catch (Throwable $e) {
...
<head>
  ...
  <title>Admin - Zamówienia żetonów</title>
  <style>
...
<div class="wrap">
  ...
  <div class="card">
    <h1 style="margin-top:0">Zamówienia żetonów</h1>
    ...
    <table>
      <thead>
        <tr>
          <th>ID</th><th>User</th><th>Pakiet</th><th>Status</th><th>Kwota</th><th>Płatność</th><th>Daty</th><th>Akcje</th>
        </tr>
      </thead>
      <tbody>
      <?php foreach ($orders as $o): ?>
        <tr>
          <td><?php echo (int)$o['id']; ?><div class="meta"><?php echo htmlspecialchars((string)$o['order_uuid'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?></div></td>
